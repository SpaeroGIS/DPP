namespace MilSpace.ArcGIS.License
{
    using System;
    ...
    public class MilSpaceToolsLicenseInitializer
    {
        ...
        ProductCode productCode = ProductCode.Engine;
        ...
        //-------------------------------------------------------------------------------------------
        public Version ArcGISVersion
        {
            get
            {
                if (arcGISVersion == null)
                {
                    arcGISVersion = typeof(IAoInitialize).Assembly.GetName().Version;
                }
...
//            : this(ProductCode.Desktop)
        ...
        {
            esriLicenseProductCode[] productCodes = new esriLicenseProductCode[] {
            ...
            Console.WriteLine("Products to initialize:");
            productCodes.ToList().ForEach(c =>
            {
                Console.WriteLine(" " + c.ToString());
            });

            esriLicenseExtensionCode[] extensionLics = new esriLicenseExtensionCode[] { esriLicenseExtensionCode.esriLicenseExtensionCodeDataInteroperability };

            Console.WriteLine("Extensions to initialize:");
            extensionLics.ToList().ForEach(c =>
            {
                Console.WriteLine(" " + c.ToString());
            });

            return InitializeApplication(productCodes, extensionLics);
        ...
        private bool Initialize()
        {
            ...
            m_requestedProducts.Reverse(); //всегда ищем самый старший продукт см. productCodes выше
            ...
            foreach (int prodNumber in m_requestedProducts)
            {
                ...
                if (productInitialized) break; //выходим на самом старшем доступном продукте
            }
        ...
        private bool m_productCheckOrdering = false; //default from low to high
        #endregion
